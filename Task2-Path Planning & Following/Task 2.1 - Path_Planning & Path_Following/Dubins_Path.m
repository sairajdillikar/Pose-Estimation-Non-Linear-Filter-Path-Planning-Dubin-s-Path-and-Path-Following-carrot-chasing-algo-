% Dubins Function provides the exit and entry angles, circles and points to
% the carrot chasing algortihm

function [exit_c,entry_c,exit_angle,entry_angle,exit_p,entry_p,min_idx] = Dubins_path(waypoints, rhos)
...
%         [Cur_pos,Cur_head_angle] = CCA_Straight(exit_p(:,c),entry_p(:,c), Cur_pos, Cur_head_angle);
        ...
        figure(1) ;
        plot( Xc1, Yc1, 'y', 'LineWidth', 1 ) ;
        hold on ;
        plot( Xc2, Yc2, 'y', 'LineWidth', 1 ) ;
        hold on ;
        xp = [TexLL(1,c), TenLL(1,c)];
        yp = [TexLL(2,c), TenLL(2,c)];
        plot(xp, yp, 'y', 'LineWidth', 1);
        hold on;
